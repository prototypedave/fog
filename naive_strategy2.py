...
if __name__ == "__main__":
    ...
    # tensorboard logger
    tensorboard_writer = SummaryWriter(str(Path(args.save_dir, "naive2")))
    ...
    for _ in tqdm(range(NUMBER_ITERATIONS), desc="Learning iterations"):
        ...
        with open(Path(args.save_dir, "naive2", "statistics.txt"), 'w') as f:
        ...
        while ep < N_EPISODES:
            ...
            if ep == 0 and ep_steps in [1, 20, 60, 200, 600, 999]:
                with open(Path(args.save_dir, "naive2", "statistics.txt"), 'a') as f:
